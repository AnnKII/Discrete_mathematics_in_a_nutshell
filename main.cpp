	...
	void VeTrongSo(int dau, int cuoi)
{
	...
	void ResetCanh()
	{
		...
		note();
			/*	if ((G[i][j]!=0) && G[j][i]!=0)
				{
					//ve canh dothi[i] => dothi[j]
					tempdau.x=dothi[i].x;
					tempdau.y=dothi[i].y;
					tempcuoi.x=dothi[j].x;
					tempcuoi.y=dothi[j].y;
					tempdau.x+=10;
					tempdau.y+=10;
					tempcuoi.x+=10;
					tempcuoi.y+=10;
					VeMuiTen(tempdau, tempcuoi, 15);
					VeDinh1(dothi[i]);
					VeDinh1(dothi[j]);
					char *temp = new char[3];
					int x,y;
					x = ((tempdau.x+tempcuoi.x) / 2);
					y = ((tempdau.y+tempcuoi.y) / 2);
					string c = IntToString(G[i][j]);
					int q = 0;
					for (; q < c.length(); q++)
					{
						temp[q] = c[q];
					...
					//ve canh dothi[j] => dothi[i]
					tempdau.x=dothi[j].x;
					tempdau.y=dothi[j].y;
					tempcuoi.x=dothi[i].x;
					tempcuoi.y=dothi[i].y;
					tempdau.x-=10;
					tempdau.y-=10;
					tempcuoi.x-=10;
					tempcuoi.y-=10;
	...
	void ResetDoThi()
{
	...
	{
		HuongDan();
		outtextxy(370, 520, "DO AN CLDL, DE 7");
		outtextxy(370, 540, "Thanh vien:");
		outtextxy(370, 560, "DONG NGUYEN ANH QUAN");
		outtextxy(370, 580, "NGUYEN HONG QUAN");
	}
	...
	/*	string s="";
		...
		sl=0;
			...
			if (kq1==true)
			{
				for (int t1=0;t1<sl;t1++)
					{
						if (kq[t1]!=dau && kq[t1]!=cuoi)
						{
							DoiMauDinh(kq[t1],2);
						}
					}
		...
		{
			outtextxy(370,520,"Ket Qua: ");
			outtextxy(370,540,&s[0]);
			outtextxy(370,560,"Press any key...");
		}
		}
		else 
